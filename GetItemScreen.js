...
export default class GetItemScreen extends Component{
  ...
  addRequest = async (name,reasonToRequest)=>{
    ...
    var books = await BookSearch.searchbook(name,'AIzaSyASyOjOtJla-X-b3io2eLoaUc_bIRFSIIc')
...
sendNotification=()=>{
  ...
  .then((snapshot)=>{
    snapshot.forEach((doc)=>{
      ...
      .then((snapshot)=>{
        snapshot.forEach((doc) => {
          var bartererIsd  = doc.data().donor_id
          var nmae =  doc.data().book_name

          //targert user id is the donor id to send notification to the user
          db.collection('all_notifications').add({
            "targeted_user_id" : bartererIsd,
...
async getItemsFromApi (bookName){
  this.setState({bookName:bookName})
    if (bookName.length >2){

    var items = await items.searchbook(items,'AIzaSyDFKhcGGg5etRe6E2OUdWRZu_MUCQczBxc')
    this.setState({
    ...
    if(this.state.IsBookRequestActive === true){
      return(
        ...
        <View style = {{flex:1,justifyContent:'center'}}>
          ...
          <Text>I recieved this item </Text>
          </TouchableOpacity>
        </View>
      )
    }
